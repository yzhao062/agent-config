...
| [prun-executor-default](2026-09-08-prun-executor-default-measurement.md) | `prun` executor routing | Breadcrumb for the 2026-09-08 flip of prun's default from Codex to Sonnet. The operative rule lives in `skills/prun/SKILL.md`; this is the searchable pointer to it. The measurement that motivated the flip was cut after four review rounds found it asserted more than it could support (cwd hash read as a run id, a switched token metric, repeated usage reports counted as turns, a mid-window snapshot read as a weekly result). | **Closed 2026-09-08**; kept as a pointer |
