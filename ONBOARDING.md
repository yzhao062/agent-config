...
1. **Pre-release checks** from a clean `main` in the `anywhere-agents` checkout:
   ...
   - Cross-repo parity: run `bash scripts/check-parity.sh` from the `agent-config` root. STRICT category (`scripts/{_python, guard.py, session_bootstrap.py, statusline.py, agent-quota.py, generate_agent_configs.py, merge_settings.py, render_banner.py, pack_identity.py, pre-push-smoke.sh, remote-smoke.sh, check-parity.sh}`, `.claude/settings.json`, `.githooks/pre-push`, `.github/workflows/{real-agent-smoke.yml, validate.yml}`, `bootstrap/bootstrap.{sh,ps1}`, `AGENTS.md` (one shared baseline since the 2026-09 rewrite; `ac`-only lines live in `ac/AGENTS.local.md`), `tests/{test_dispatch_codex.py, test_dispatch_copilot.py, test_dispatch_claude.py, test_health_check.py, test_guard.py, test_session_bootstrap.py, test_render_banner.py, test_bootstrap_size.py, test_pointer_files.py, test_prompt_byte_parity.py, test_bootstrap_preflight.py}`, `skills/{implement-review, ci-mockup-figure, readme-polish, prun, editable-figure}` recursive) must be byte-identical. BY-DESIGN category (`user/settings.json`, `skills/my-router`) reports a +/- line delta per file; a byte-for-byte match warns that sanitization may have been skipped. The 6 shipped `.claude/commands/*.md` pointers, plus the `vet.md` alias, were dropped from cross-repo STRICT in v0.4.0 (pack-emitted; see `pack-architecture.md` § "STRICT parity trajectory"). Exit 0 means STRICT clean and every BY-DESIGN mirror present; exit 1 means drift or a missing required mirror.
...
- `skills/{bibref-filler,bibref-verify,figure-candidate-gallery,figure-prompt-builder,venue-review-panel}/` — active skill content for the `acad-skills` pack; remote installation via `anywhere-agents` v0.5.0+ direct-URL pack consumption
