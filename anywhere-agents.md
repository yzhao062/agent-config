...
| `scripts/render_banner.py`, `scripts/pack_identity.py` | Yes, as-is (STRICT) | Session banner renderer and its read-only pack-identity helper. Both entry points and the hook run them from the sparse clone; Phase 3 must vendor the pair into the wheel so both delivery paths use the same calculation. |
