...
**Shared-contract test files** (added to STRICT after the v0.6.0 column above, so omitted from the snapshot grid for brevity): `tests/test_guard.py`, `tests/test_dispatch_codex.py`, `tests/test_health_check.py`, and `tests/test_prompt_byte_parity.py` joined the STRICT set on 2026-05-16, and `tests/test_bootstrap_preflight.py` in v0.7.0. They pin the runtime contract of the STRICT shared scripts (guard, dispatch-codex, health-check, bootstrap preflight) and are enforced by the `strict_test_files` array in `check-parity.sh`, so a shared-contract change proposed in either repo must mirror its tests in the same commit. At v1.0.0 `tests/test_guard.py` follows `guard.py` into `agent-behave` and drops from the cross-repo STRICT list; the other four stay STRICT.
