...
- **Per-guard escape hatch env vars**: `AGENT_STYLE_HOOK=off`, `AGENT_BEHAVE_HOOK=off`, etc. Useful when the user is writing meta-discussion content (e.g., the style-guide document itself, which legitimately quotes banned words as examples of what to avoid). The blanket `AGENT_CONFIG_GATES=off` stays as the emergency switch.
- **Composer noise budget gate** at install time. Still serves third-party packs that may stack `false-positive-risk: high + impact-if-allowed: low|medium + decision: deny` entries **without** reroute hints — the gate forces the consumer to acknowledge via `decision-override: deny` in `agent-config.yaml`. First-party packs post-v0.7.0 still produce zero budget-tripping entries by design, because every new first-party `deny` ships with a reroute hint and therefore is not counted as budget noise (Round 6 sharpened definition).
