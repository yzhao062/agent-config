  ...
  - name: agent-style
    ...
    active:
      - kind: hook
        ...
        decision: deny                         # Round 6: deny + reroute_hint stays
        reroute_hint: "rephrase without the banned word (close variants caught too)"
        trigger-rate: high
  ...
  - name: agent-behave
    ...
    active:
      ...
      - kind: hook
        ...
        match: [cd * && *, cd *; *]
        decision: deny                         # Round 6: deny + reroute_hint stays
        reroute_hint: "use `git -C <path> <cmd>` for git, or pass the path as an argument"
        trigger-rate: high
        false-positive-risk: high
...
- `source_input_sha256` (generated-command only): in the shipped composer, the rendered pointer's hash, the same value as `output_sha256` (`scripts/packs/handlers/skill.py`). The original design copied the referenced active entry's `input_sha256` here; since the composer renders the pointer on every dispatch, no decision reads this field.
...
The composer reads this file on every bootstrap; mismatch between recorded `input_sha256` and current upstream content, or between recorded `resolved_commit` and current source tip, triggers either the `locked` fail-closed path or the `auto` refresh path depending on `effective_update_policy`. For `generated-command` entries, the composer renders and stages the pointer on every dispatch. The drift gate compares the pointer on disk with the recorded `output_sha256`, and the transaction skips a write whose bytes already match, so an unchanged pointer is never rewritten.
...
**`ac` / `aa` impact**: none at this release — `aa`'s built-in `guard.py` still handles the full risk classifier (destructive git / gh, package publish, file/device destruction, compound-cd) across Bash + PowerShell with wrapper-piercing (see v0.7.1). `ab` is an opt-in addition, not a replacement.
...
- **Update-UX revisit — collapsed by the minimal-command-surface principle.** v0.5.6 closed the wheel-delivers-fix gap, but the day-to-day pack-update flow is still split across multiple commands with partially-documented policy semantics. v0.5.x shipped the primitives (`update_policy: {locked, prompt, auto}`, `pack update`, `pack verify --fix`, `latest_known_head` drift detection in the banner) and deliberately deferred the UX layer that ties them together.

  **Design principle: minimal command surface.** The v0.5.x trajectory already collapsed several CLI surfaces (v0.5.2 collapsed verify-then-bootstrap; v0.5.4 auto-ran `pack verify --fix --yes` after bootstrap; v0.5.8 made `--fix` materialize declared state). v0.6.0 names that pattern explicitly and finishes it: **bare `anywhere-agents` is the universal apply command.** Bootstrap if needed, deploy declared state, apply prompt-policy drift on mutable refs, regenerate `CLAUDE.md`. The user's mental model is one verb: "I run this and my project is in the desired state." `pack verify --fix` and `pack update` are flavors of the same operation; in v0.6.0 they become **compatibility aliases** that print a one-line stderr notice pointing at `anywhere-agents` and otherwise execute the canonical apply path. Aliases remain supported through all v0.x releases (the v0.5.x line trained users on `pack verify --fix --yes` as a documented heal command and that scripted CI usage cannot be broken on a one-minor-release timeline); removal is allowed at v1.0 only with explicit CI-migration guidance, and if the maintenance cost stays negligible the aliases may persist as hidden compatibility shims after v1.0 as well. "Deprecated" here means "not taught," not "broken next minor." Most consumers should never need to type more than two or three commands; multiplying verbs to handle edge cases pushes cognitive load to the wrong side.

  User-facing surface after v0.6.0:

  ```
  anywhere-agents                    # apply: bootstrap + deploy + drift + generator
  anywhere-agents --dry-run          # inspect: same logic, no writes
  anywhere-agents pack add <src>     # declare
  anywhere-agents pack remove <name> # declare
  anywhere-agents pack list          # see declared state
  anywhere-agents uninstall --all    # power-user reset (rare)
  ```

  READMEs and banners only instruct users to run `anywhere-agents` for the apply path. `pack verify` (no `--fix`) becomes `anywhere-agents --dry-run`, or stays as a thin alias for inspection-only output. Selective application (`pack update <pack>`) survives as a power-user verb only if real demand surfaces.

  The five questions originally framed for v0.6.0 plan-review resolve through this principle:

  - **Q1** (`pack verify --fix` apply prompt-drift, vs `pack update`-only route): both collapse into bare `anywhere-agents`. The canonical command applies prompt-policy drift inline by default and prints a stderr summary line (`applied 1 update for agent-pack @ main: <old> -> <new>`). The default can be disabled per run with `ANYWHERE_AGENTS_UPDATE=skip` (the v0.5.0 escape hatch defined at the "Source resolution and active-code trust" section above) and, if a CLI flag is added, `anywhere-agents --no-apply-drift`; those modes leave drift reported but unapplied. Consumers wanting a durable fail-closed policy pin `update_policy: locked` in `agent-config.yaml`. CI / non-interactive contexts inherit default apply only when no skip override is set. Interactivity is not reintroduced.
  - **Q2** (banner follow-up command): no new verb. The follow-up is "run `anywhere-agents`".
  - **Q3** (bundled-default `update_policy`): the canonical command must DTRT without a user decision. The v0.6.0 bundled-default policy table is settled here, not punted to the PLAN file:

    | Pack class | Example | Default `update_policy` | Rationale |
    |---|---|---|---|
    | First-party passive | `agent-style` | `auto` (silent refresh + stderr summary on changes) | Passive text only; the wheel pins the bundled ref. v0.5.7 / v0.5.8 five-project sandbox is sufficient evidence. |
    | First-party active | `aa-core-skills` | `prompt` (apply-by-default per Q1 + stderr summary) | Active code, hooks, skill content. Apply-by-default is acceptable because the wheel pins the bundled ref and the apply path emits an audit trail. |
    | Third-party passive | `agent-pack/profile` | `prompt` (apply-by-default per Q1 + stderr summary) | Mutable refs are common in third-party packs; the audit trail is more important. |
    | Third-party active | `agent-pack/acad-skills` | `prompt` (apply-by-default per Q1 + stderr summary). **Never default `auto`.** | Silent install of arbitrary code from a third-party mutable ref is the supply-chain risk `prompt` was designed to gate. |

    Consumers pin `update_policy: locked` in `agent-config.yaml` for any pack where they want fail-closed behavior; the table is the *default*, not the only option.
  - **Q4** (`pipx upgrade anywhere-agents` hint): out of scope for the consumer-facing surface. Maintainer-side; not a banner item or session-start hint.
  - **Q5** (`update_policy: auto` on active entries): re-reject at parse time in v0.6.0. This is doc-coherence repair, not new policy: the trust-model paragraph above (§ "Source resolution and active-code trust") and the churn-semantics paragraph already state "Active entries never use `auto`; attempting to set it on an active entry is a manifest error." v0.5.0 silently removed the parser check; v0.6.0 restores it. The parse error names the pack, the active entry's `files[].to` path, the offending `update_policy: auto`, and the required rewrite (`prompt` for default-apply behavior, `locked` for fail-closed); a fixture in `tests/test_packs_schema.py` pins error-message readability. The v0.6.0 CHANGELOG entry serves as both the announcement and the change, justified as restoring an already-documented manifest error rather than introducing new policy — there is no earlier v0.5.x release left in this plan to carry a separate warning, so the rejection ships in the same release that records it. Migration path for any pre-existing active `auto`: maintainer-project scan before tagging plus the actionable parse error. If a real consumer is found, that consumer is contacted with the rewrite before v0.6.0 ships, with deferral of the hard rejection limited to that consumer's path; otherwise the rejection ships unconditionally.
...
**Consumer-facing change**: visible. Bare `anywhere-agents` becomes the canonical apply command and absorbs `pack verify --fix` + `pack update` as compatibility aliases through all v0.x (CI scripts continue to work; aliases print a stderr notice pointing at `anywhere-agents`). Prompt-policy drift on mutable refs applies inline by default with a stderr summary line, with `ANYWHERE_AGENTS_UPDATE=skip` and `--no-apply-drift` available as per-run overrides. `update_policy: auto` on active entries is re-rejected at parse with an actionable error message naming pack + `files[].to` + required rewrite. The `random`-style failure mode (auto-reconciled minimal entry blocks bundled-default migration) and the v0.5.7 same-ref source-path switching commitment both ship in this release. CHANGELOG enumerates the alias notice, inline-apply default, schema rejection, and the BC-guard refinement.

**STRICT parity**: aa-internal STRICT block extends `scripts/check-parity.sh` for the wheel-bundled mirror. Cross-repo `guard.py` STRICT membership is unchanged in this release; noise-audit reroute-hint upgrades (Round 6) ship in v0.7.0.

**Shipped 2026-05-04** as one bundled commit on origin/main (`0de7482`) tagged `v0.6.0`; PyPI and npm published in lockstep. One post-review fix beyond the original eight-phase scope: host-aware bundled-default seeding. The maintainer-project sandbox under `AGENT_CONFIG_HOST=codex` surfaced that v0.5.x and pre-fix v0.6.0 both auto-seeded `aa-core-skills` (which declares `hosts: [claude-code]`) regardless of host, so codex consumers hit a host-mismatch error on first run; v0.6.0's promotion of bare `anywhere-agents` to canonical made the trap visible on the first command a fresh codex user types. The fix adds `_default_v2_selections_for_host(host)` in `scripts/compose_packs.py` and a matching `_default_v2_seed_for_host(host)` + `_active_host()` pair in `packages/pypi/anywhere_agents/cli.py`, filtering claude-only entries from the auto-seed when the active host is not `claude-code`. The full `DEFAULT_V2_SELECTIONS` list stays canonical for identity-check, BC-guard, and drift-detection call sites, so a user-pinned `aa-core-skills` row under codex still resolves to the synthetic bundled identity. New `_CLAUDE_ONLY_DEFAULTS` constant in both files centralizes the host-restricted-default mapping; a future host-restricted bundled default needs only one entry update there. Removes the v0.5.x `AGENT_CONFIG_PACKS=-aa-core-skills` workaround.

**Real-project verification**: 5 maintainer projects (usc-admin, usc-email, usc-slides, random, yzhao062.github.io) converge to coherent state with exit 0 on Windows. Spark Ubuntu (aarch64, Python 3.12.3) greenfield bootstrap end-to-end success with 859 substantive tests passing on Linux. Full Windows test suite at 919 passed across the v0.6.0 line (901 baseline + 14 host-gate tests + 4 from intermediate review-loop additions). Five Codex review rounds total: four on the original eight-phase implementation, one on the host-gate post-review fix.

### aa v0.7.0 — Noise audit

**Scope revised in Round 6** based on autonomous-mode lived experience: the original "high FP → demote `deny` to `ask`" reflex is wrong when the guard has an obvious agent-side reroute, because `ask` blocks the autonomous loop indefinitely while `deny` lets the agent course-correct in one extra model turn. Real criterion: **does an obvious agent-side reroute exist?** If yes, keep `deny` and surface the reroute hint inline. If no (destructive git, destructive gh, immutable publishes), `ask` is correct and already in place.

Concrete v0.7.0 scope:

- **Keep `deny` defaults on writing-style and compound-cd hooks** (the two candidates originally named for demotion). Both have clean agent-side reroutes — rephrase the banned word; use `git -C <path>` instead of `cd <path> && git ...`. Demoting them to `ask` would convert a one-turn agent course-correction into an indefinite human-approval block during `/implement-review auto` runs, long-running `claude -p` jobs, and any other unattended loop. The prior `false-positive-risk: high + impact-if-allowed: low|medium` heuristic is superseded by the reroute criterion (see Round 6).
- **Upgrade deny messages to embed the reroute inline.** Current writing-style deny prints just the hit list; the new shape names a concrete alternative (e.g., "rephrase 'encompass' as 'cover' / 'include' and retry"). Current compound-cd deny prints the suggestion as prose; the new shape elevates it to a structured `Suggested rewrite:` line that the model can lift verbatim. Reduces reroute latency from "infer a workaround" to "lift the suggested rewrite".
- **Per-guard escape hatch env vars**: `AGENT_STYLE_HOOK=off` disables the writing-style gate; `AGENT_COMPOUND_CD_HOOK=off` disables the compound-cd gate. Useful when the user is writing meta-discussion content (e.g., the style-guide document itself, which legitimately quotes banned words as examples of what to avoid). The blanket `AGENT_CONFIG_GATES=off` stays as the emergency switch for writing-style + banner. Destructive git/gh stay always-on under every escape env (no agent-side reroute exists). The advertised set lives in `scripts/guard.py:_ESCAPE_HATCH_ENV_NAMES`; a static literal-scan test in `tests/test_guard.py` enforces that no future hook env var can bypass the destructive-non-bypass invariant without registering in that constant.
- **Composer noise budget gate** at install time. Warns (does not hard-fail) for third-party hook entries with `false-positive-risk: high + impact-if-allowed: low|medium + decision: deny + missing reroute_hint`. Consumer per-pack override is `noise-audit-override: accept-deny` in `agent-config.yaml`, which silences the warning for that specific pack. First-party packs post-v0.7.0 ship every `deny` with a `reroute_hint` and therefore are not counted as budget noise (Round 6 sharpened definition). The gate prints warnings via the existing compose summary; it does not block composition.

**Consumer-facing change**: visible but smaller than originally planned. Users keep seeing `deny` on banned-word writes and `cd ... && ...` chains; the visible change is **more useful deny messages** with a concrete `Suggested rewrite:` line, plus the new escape env vars. CHANGELOG highlights the autonomous-mode rationale so consumers understand why deny stayed.

**Budget gate scope**: unchanged in spirit from Round 2's `> 0` threshold; Round 6 sharpens "noise" so that a `deny` carrying a concrete `Suggested rewrite:` line is not counted as budget noise, only a bare `deny` without one is.

**STRICT parity**: `guard.py` is STRICT byte-identical between ac and aa. Round 6 message and env changes land in both. `check-parity.sh` continues to enforce byte-identical until v1.0 extraction.

**Why split from v0.6.0**: v0.6.0 closed update-flow coherence. The Round 6 scope (reroute hints + escape envs + budget gate) is independent code surface (`guard.py` message formatting + composer install gates) with no concrete user-pain trigger today; combining with v0.6.0 would have lost the "one visible concern per release" discipline.

**Shipped 2026-05-19**, tagged `v0.7.0`, across ac (`f8c62f7` Slice A, `f270eea` Slices B+C, `158e140` CI hotfix) and aa (`c557ce4` Slice A, `2e2e34f` Slices B+C, `60b2e49` CI hotfix, `ae645f6` release prep). The release bundled two adjacent slices with the noise audit: **Slice B** promoted the agent-fungibility principle from `CLAUDE.local.md` to shared `AGENTS.md` § Agent Fungibility (Phase 0.5 of the fungibility refactor), and **Slice C** added a `bootstrap.{sh,ps1}` `git --version >= 2.25` preflight that turns a real consumer's cryptic `unknown option 'sparse'` failure into a platform-specific install line. PyPI + npm published in lockstep: PyPI via `twine upload`, npm via a new `.github/workflows/publish.yml`. The npm leg surfaced that the package's publishing-access policy requires 2FA on publish, which rejects automation tokens with E403; the durable fix is tokenless **OIDC trusted publishing** (npm + PyPI both OIDC-capable now), recorded in `docs/followups/2026-05-20-aa-publish-workflow-oidc.md`. Implementation reviewed via `/implement-review` (Codex Auto-terminal): Slice A converged in 5 rounds (R1 caught dead-code wiring + a 3-copy schema mirror gap + a quote-blind compound-cd parser; R5 clean), Slices B+C in 3 rounds (R1 caught a POSIX test PATH-stripping fail-open; R3 clean). A post-merge CI hotfix fixed a macOS / Linux fail-open in `test_bootstrap_preflight.py` (`/usr/local/bin` ships git on CI runners). Validation: full ac + aa unit suites, `check-parity.sh` STRICT clean, end-to-end bootstrap-preflight smoke with a real `bash bootstrap.sh` against stub git 2.24 / missing / 2.25 on both the fresh-clone and existing-repo paths, plus a wheel-delivery smoke on a scratch consumer. Spark dual-OS validation handed off as a manual maintainer step.

### aa v0.7.1 — Tool-agnostic guard

A patch follow-on to v0.7.0, not a new architecture surface. `guard.py` becomes tool-agnostic: the same risk classifier runs for the `PowerShell` tool as well as `Bash`, and the user allow-list pairs `PowerShell(*)` with `Bash(*)` so the native permission layer is allow-by-default and the hook is the sole risk arbiter on both shells. The ask set gains package publish (`npm`/`pnpm`/`yarn publish`, `twine upload`, `python -m twine upload`), `gh release create/delete/upload/edit`, and PowerShell recursive `Remove-Item` (+ aliases) deletes. Classification keys on the exact leading token per sub-command, strips transparent prefix runners (`sudo`, `doas`, `env`, `command`, `nohup`, `setsid`), and sees through command-carrying wrappers (`ssh`, `bash`/`sh`/`zsh -c`, `docker exec`/`run`, Windows `cmd /c`, `timeout`, `xargs`) up to `MAX_WRAPPER_DEPTH`.

**Consistency with Round 6**: the new ask classes confirm the reroute criterion rather than revise it. Package publish and file/device destruction have no agent-side reroute, so `ask` is correct (human approval is the contract); there are no `deny` → `ask` demotions, and the mandatory ask set is non-bypassable by any env var. Obfuscation (base64, `$(...)`, here-docs) and the low-frequency prefixes `nice`/`ionice`/`stdbuf`/`time` plus `python -c` are documented non-goals: the guard defends against honest mistakes, not deliberate evasion.

**STRICT parity**: `guard.py` and `tests/test_guard.py` stay byte-identical ac ↔ aa (290 guard tests, Windows + ARM64 Linux); `check-parity.sh` STRICT clean. The v1.0 extraction-to-`agent-behave` plan is unchanged; this release only broadens what `guard.py` classifies internally.

**Shipped 2026-05-21**, tagged `v0.7.1` (ac `e5ea295`; aa feature `186a89f` + release `a859540`). PyPI + npm published via OIDC trusted publishing; npm CDN propagation lagged the Package Smoke retry budget once and needed a manual rerun. Implementation converged in 5 `/implement-review` (Codex Auto-terminal) rounds: R1–R3 caught path-qualified git/gh, `rm -Rf` variants, wrapper option forms, and npm / versioned-python publish-detection gaps; R4 caught a PowerShell `-Command` single-token bypass plus the realistic prefix-runner boundary (`command` / `timeout` / `xargs` in, `nice` / `ionice` / `stdbuf` / `time` documented out); R5 clean.

### aa v1.0.0 — Full decoupling

- `guard.py`'s risk-classifier logic (destructive git / gh, package publish, file/device destruction, compound-cd; tool-agnostic Bash + PowerShell; wrapper recursion) extracts into `agent-behave`.
...
| aa v0.6.0 | None (no bootstrap script change expected; `pipx upgrade anywhere-agents` stays maintainer-side per Q4 disposition) | Document the canonical-apply collapse: bare `anywhere-agents` absorbs `pack verify --fix` + `pack update` as compatibility aliases retained through all v0.x; document inline prompt-policy drift apply default with stderr summary, `ANYWHERE_AGENTS_UPDATE=skip` / `--no-apply-drift` overrides; document the v0.6.0 bundled-default policy table; document `update_policy: auto`-on-active rejection (parse error format + release-note warning + `tests/test_packs_schema.py` fixture); document reconciliation-aware BC-guard refinement plus the bundled same-ref source-path switching path; document the new aa-internal STRICT block in `check-parity.sh` | Rewrite quickstart to lead with the canonical `anywhere-agents` command; demote `pack verify --fix` and `pack update` to a "Legacy aliases" footnote (note that they continue to work and will not be removed before v1.0); update FAQ for the inline-apply default + per-run overrides and the BC-guard behavior change; update `docs/anywhere-agents-quickstart.md` § "Common gotchas" with the new schema-rejection error and the same-ref-path migration |
| aa v0.7.0 | None unless `guard.py` hook wiring changes in bootstrap | Document per-guard escape env vars in "Mechanical Enforcement"; document the Round 6 reroute criterion (`deny` stays when an agent-side reroute exists); document the reroute-hint message pattern (`Suggested rewrite:` line in deny output) and the third-party-pack noise-budget threshold | Update FAQ for consumer-visible message changes (denies now embed a `Suggested rewrite:` line) — no behavior demotion from `deny` to `ask` |
...
|---|---|---|---|---|---|
| `scripts/guard.py` | STRICT | STRICT | STRICT | STRICT (v0.7.0 reroute-hint upgrades; v0.7.1 tool-agnostic classifier + PowerShell, still byte-identical in both) | **DROPPED** (extracted to ab) |
...
**Shared-contract test files** (added to STRICT after the v0.6.0 column above, so omitted from the snapshot grid for brevity): `tests/test_dispatch_codex.py`, `tests/test_dispatch_copilot.py`, `tests/test_health_check.py`, `tests/test_guard.py`, `tests/test_session_bootstrap.py`, `tests/test_pointer_files.py`, `tests/test_prompt_byte_parity.py`, and `tests/test_bootstrap_preflight.py` are the current STRICT set, in the same order as the `strict_test_files` array in `scripts/check-parity.sh`. `tests/test_dispatch_codex.py`, `tests/test_health_check.py`, `tests/test_guard.py`, and `tests/test_prompt_byte_parity.py` joined on 2026-05-16; `tests/test_bootstrap_preflight.py` joined in v0.7.0; `tests/test_dispatch_copilot.py` joined when the GitHub Copilot reviewer backend shipped; `tests/test_session_bootstrap.py` joined with the issue #7 banner-re-fire fix (compact-skip + source-aware debounce); and `tests/test_pointer_files.py` joined with the issue #6 pointer-lookup-gap fix (3-path lookup including `.claude/skills/<name>/`). They pin the runtime contract of the STRICT shared scripts (dispatch-codex, dispatch-copilot, health-check, guard, session_bootstrap, bootstrap preflight) plus the on-disk shape of every committed `.claude/commands/*.md` pointer, and are enforced by the `strict_test_files` array in `check-parity.sh`, so a shared-contract change proposed in either repo must mirror its tests in the same commit. At v1.0.0 `tests/test_guard.py` follows `guard.py` into `agent-behave` and drops from the cross-repo STRICT list; `tests/test_session_bootstrap.py` follows `scripts/session_bootstrap.py` on the same trajectory if the v1.0 plan-review relocates the SessionStart-hook half of the banner system together with the guard. The remaining files stay STRICT.
...
1. `trigger-rate: high + decision: deny` without a reroute hint blocks autonomous agents on every false-positive turn. Mitigation: Round 6 noise audit (v0.7.0) requires `reroute_hint:` on every first-party + third-party hook with this profile so the agent can lift the reroute in one model turn; the composer noise-budget gate warns on third-party packs that omit it. Users facing legitimate meta-discussion blocks set the per-guard escape env (`AGENT_STYLE_HOOK=off`, `AGENT_COMPOUND_CD_HOOK=off`).
...
3. Writing-style gate false positives on meta-discussion documents (style guides, CHANGELOGs that quote banned words). Mitigation: code-fence + inline-backtick stripping already in `guard.py` eliminates the common pattern; `AGENT_STYLE_HOOK=off` (per-guard escape env from v0.7.0) covers residual cases; per-file-name whitelist in manifest (e.g., `exempt: [STYLE.md, CHANGELOG.md]`) is a possible extension.
...
15. v0.7.0: writing-style and compound-cd denies stay as `deny` but the deny message embeds a concrete `Suggested rewrite:` line; autonomous agent re-rephrases without human intervention. Per-guard escape envs (`AGENT_STYLE_HOOK=off`, `AGENT_COMPOUND_CD_HOOK=off`) bypass the respective guards in meta-discussion contexts; destructive git/gh stay always-on (no env can disable them). Composer noise-budget gate warns on third-party hook entries with `decision: deny + false-positive-risk: high + impact-if-allowed: low|medium + missing reroute_hint`.
...
28. **v0.6.0 same-ref source-path switching smoke** (gates v0.6.0 itself and any later release that touches the BC-guard or bundled-default flip path): set up a fixture project that reproduces the auto-reconciled minimal-entry case — `agent-config.yaml` carries a stale `source.ref: v0.3.2` for `agent-style` with no `passive` / `active` / `update_policy` keys; `.agent-config/pack-lock.json` carries a stale `requested_ref: v0.3.2` and stale `source_path: docs/rule-pack.md`; the deployed `AGENTS.md` reflects the full-body content. Run bare `anywhere-agents`. Pass when the project converges to compact across all three artifacts: `agent-config.yaml` no longer carries the stale `source.ref` for the auto-reconciled minimal entry; `.agent-config/pack-lock.json` advances to the bundled-default ref and `source_path: docs/rule-pack-compact.md` with no stale `requested_ref`; `AGENTS.md` size matches the v0.5.7 compact target (~70 KB on a maintainer-style project). Override-preservation as a separate clause: a parallel fixture with genuine user-authored `passive` / `active` / `update_policy` keys must NOT be silently rewritten — entries the BC-guard refinement classifies as deliberate pins survive the canonical-apply run. Fail when the lock, yaml, or `AGENTS.md` stays on full-body, or when a user-authored override is overwritten. Reproduces the v0.5.7 / `random` real-project asymmetry that motivated the bundling decision in the first place; checklist-style validation against synthetic fixtures was not enough at v0.5.x and should not be enough at v0.6.0.
...
- **`guard.py` extraction timing** (was #10): stays at v1.0.0. Folding it into v0.7.0 would couple two visible consumer changes (Round 6 reroute-hint deny upgrades + deploy-pattern change) in one release; keep them separate.
...
- **Noise budget threshold** (was #7): v0.7.0 threshold is `> 0`. Any combined install that produces at least one `false-positive-risk: high` + `impact-if-allowed: low|medium` + `decision: deny` + missing `reroute_hint` entry from a third-party pack emits a composer warning in the compose summary. The warning is non-blocking; consumers silence per-pack via `noise-audit-override: accept-deny` in `agent-config.yaml`. First-party packs post-v0.7.0 produce zero such entries by design (every first-party `deny` ships with a `reroute_hint`), so the gate mainly serves third-party stacking.
...
**Round 5 decisions** (v0.6.0 update-flow scope):

- **Bare `anywhere-agents` is the canonical apply command** (was Q1 + Q2 + open scope). `pack verify --fix` and `pack update` collapse into it as compatibility aliases in v0.6.0. READMEs and banners only instruct users to run the bare command for the apply path, and alias invocations print a one-line stderr notice pointing to `anywhere-agents`. The aliases remain supported through all v0.x releases and may be removed only at v1.0 with explicit CI-migration guidance; if the maintenance cost stays negligible, keep them as hidden compatibility aliases after v1.0. The banner's "N updates available" follow-up is "run `anywhere-agents`", not a new verb. The user-facing surface after v0.6.0 is six items total: `anywhere-agents`, `pack add` / `remove` / `list`, `uninstall --all`, plus the `--dry-run` flag. The motivating principle: 90% of consumers should not have to memorize more than two or three day-to-day commands; multiplying verbs to handle edge cases pushes cognitive load to the wrong side.
- **Inline prompt-policy drift application by default** (was Q1). Bare `anywhere-agents` applies prompt-policy drift on mutable refs and prints a stderr summary. Per-run skip remains available through `ANYWHERE_AGENTS_UPDATE=skip` and `anywhere-agents --no-apply-drift`; those modes report drift but do not apply it. Consumers wanting durable fail-closed behavior pin `update_policy: locked` in `agent-config.yaml`. CI / non-interactive contexts inherit default apply only when no skip override is set. Interactivity is not reintroduced.
- **Q3, Q4, Q5 details** are settled in this v0.6.0 section, not deferred. Bundled defaults use the policy table inside § "aa v0.6.0 — Update-flow coherence" (Q3 paragraph): first-party passive `agent-style` defaults to `auto` with a stderr summary, first-party active `aa-core-skills` defaults to `prompt` with apply-by-default plus summary, third-party passive defaults to `prompt`, and third-party active defaults to `prompt` and never `auto`. `pipx upgrade anywhere-agents` stays maintainer-side. `update_policy: auto` on active entries is re-rejected at parse for schema clarity, with an error that names the pack, active `files[].to` path, offending policy, and required rewrite to `prompt` or `locked`.

**Round 6 decisions** (v0.7.0 noise-audit scope revision, based on autonomous-mode lived experience):

- **Reroute criterion supersedes the FP-rate criterion for `deny` → `ask` demotions.** The prior heuristic — `false-positive-risk: high + impact-if-allowed: low|medium → demote to ask` — assumed a human in the loop who benefited from being asked rather than blocked. In autonomous flows (`/implement-review auto` dispatching Codex via background subprocess; long-running `claude -p` jobs; any unattended review loop), the asymmetry flips: `deny` lets the agent see a hard NO and course-correct in one extra model turn, while `ask` halts the loop indefinitely waiting for human approval. The two demotion candidates previously named (writing-style hook, compound-cd hook) both have clean agent-side reroutes (rephrase the banned word; use `git -C`), so both stay as `deny`. New criterion: **does an obvious agent-side reroute exist?** If yes, keep `deny` and surface the reroute inline; if no (destructive git / gh, immutable publishes), `ask` is correct and already in place.
- **v0.7.0 scope rewrite**: reroute-hint message format (`Suggested rewrite:` line in deny output), per-guard escape env vars, and the third-party composer noise budget gate. No demotions. See § "aa v0.7.0 — Noise audit" for the full rewrite.
- **Round 1 `guard.py` v1.0.0-extraction-timing decision retains its rationale** even though v0.7.0 no longer demotes. The "one visible concern per release" discipline still motivates keeping the extraction at v1.0; the Round 1 wording does not require rewording.
- **Round 2 noise-budget threshold (was #7) wording retains the `> 0` semantics**: the gate still triggers when a third-party install produces any high-FP + low/medium-impact + `deny` entry. Round 6 sharpens what "noise" means — a `deny` carrying a concrete `Suggested rewrite:` line is not budget noise (the agent can reroute in one turn), only a bare `deny` without one is. First-party packs post-v0.7.0 ship every `deny` with a reroute hint, so the gate still mainly serves third-party stacking.

**Round 7 decisions** (Codex skill links; plan review of 2026-09-24, ten rounds with Codex and Agy):

- **Codex skill links are a derived view outside pack-lock.** Codex discovers repository skills only in `.agents/skills/`. After each committed compose, `scripts/packs/codex_links.py` links `.agents/skills/<name>` to `../../.claude/skills/<name>` for every staged skill whose effective hosts include `codex`, under the same repo lock. A link carries no content hash; each compose checks it by target text and ownership. The transaction keeps its file-only operations (Round 3). Uninstall never hands a link to `_delete_project_local`, whose `resolve()` plus `shutil.rmtree` would follow it into `.claude/skills`.
- **Ownership lives in the nested, self-ignoring `.agents/skills/.gitignore`.** The file has a fixed header and grammar, and a name is owned only when the file listed it at the start of a run or the run created the link. Tracked paths, case variants, occupied names, and unowned links are reported and never replaced or removed; one `git ls-files -- ':(icase).agents'` query supplies the tracked names. Unknown git tracking, a symlinked or tracked parent or record, or a malformed record leaves `.agents` byte-for-byte unchanged until a later compose. The record is written before any new link, so a crash leaves at most a listed name that the next compose reconciles.
- **Lifecycle.** Every compose that runs the transaction creates, keeps, removes, or reports links. The opt-out path leaves links in place with their targets. Both uninstall entry points remove owned links whose target directory is gone, on every run that finds the record, including a retry that finds no pack state. A skipped update dispatches the newly fetched definition with the locked files, so the new hosts decide its links; that skip behavior predates this decision and is tracked separately.
- **Hosts and platforms.** The skill handler collects eligibility only for entries that dispatch accepted, so the Round 2 host-mismatch semantics are unchanged. `aa-core-skills` and agent-pack's `acad-skills` declare `hosts: [claude-code, codex]`, and `_CLAUDE_ONLY_DEFAULTS` is empty. Links are created on macOS and Linux only. Windows reports the limit whenever Codex-eligible skills exist, and a verified junction or copy design is later work, as is the Codex SessionStart hook.
...
| aa v0.6.0 | Update-flow coherence | **Bare `anywhere-agents` becomes the canonical apply command**; `pack verify --fix` + `pack update` collapse into compatibility aliases (retained through all v0.x; removable only at v1.0 with explicit CI-migration guidance); inline prompt-policy drift apply by default with stderr summary, `ANYWHERE_AGENTS_UPDATE=skip` / `--no-apply-drift` per-run overrides; v0.6.0 bundled-default policy table (first-party passive `auto`, all active `prompt`); `update_policy: auto` re-rejected on active entries with actionable parse error; reconciliation-aware BC-guard refinement bundles same-ref source-path switching (`random` + v0.5.7 caveat); aa-internal STRICT block in `check-parity.sh` | Visible: collapsed CLI surface, alias notice on legacy commands, `agent-pack @ main`-style drift applies on canonical run, BC-guard fix delivers v0.5.7's deferred same-ref switching commitment |
| aa v0.7.0 | Noise audit (Round 6 reroute criterion) | `guard.py` deny messages embed concrete `Suggested rewrite:` lines; narrow per-guard escape envs (`AGENT_STYLE_HOOK`, `AGENT_COMPOUND_CD_HOOK`) with destructive git/gh always-on; composer noise-budget gate counts third-party hooks with `decision: deny` + `false-positive-risk: high` + `impact-if-allowed: low\|medium` + missing `reroute_hint`; new optional `reroute_hint` schema field; bootstrap `git --version >= 2.25` preflight; fungibility principle promoted to shared AGENTS.md (Phase 0.5) | Visible: deny messages now actionable; new escape envs documented; preflight turns cryptic git-too-old failure into install instruction |
| aa v0.7.1 | Tool-agnostic guard | `guard.py` classifies for `Bash` + `PowerShell` (allow-list pairs `PowerShell(*)` with `Bash(*)`); ask set gains package publish, `gh release`, and PowerShell recursive deletes; leading-token classification with wrapper-piercing + transparent-prefix stripping; mandatory set non-bypassable; obfuscation + low-frequency prefixes documented non-goals | Visible: PowerShell stops prompting except on the dangerous set |
