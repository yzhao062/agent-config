...
|-------------------|------------------------------|-------|
| `AGENTS.md` | Yes, after sanitization | Strip USC-specific content, Overleaf rules, PyCharm-specific paths, NSF-DEI stance, submodule workflow, etc. Keep the curated defaults (writing style, Git safety, shell style). Replace specific paths with placeholders. Generalize User Profile. **Preserve `<!-- agent:claude -->` / `<!-- agent:codex -->` tag markers intact** — the generator depends on them. |
