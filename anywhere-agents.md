...
| `yzhao062/anywhere-agents` | **Sanitized public release** | Public | Only what a stranger can fork and use: clean `AGENTS.md`, six shared skills (`implement-review`, `my-router`, `ci-mockup-figure`, `readme-polish`, `prun`, `editable-figure`), bootstrap scripts, guard hook, settings, tests, CI. Includes the maintainer-selected figure preferences and locators described in the `skills/editable-figure/` row below; research-specific reference skills remain excluded. |
...
|-------------------|------------------------------|-------|
| `AGENTS.md` | Yes, byte for byte (STRICT) | One shared baseline since the 2026-09 rewrite (`docs/followups/2026-09-17-agents-md-diet.md`). Nothing personal is left to strip: the profile, Overleaf rules, and submodule workflow live in the `agent-pack` packs, and the maintainer lines an `ac` session needs (preferred interpreter, PyCharm default, the `ONBOARDING.md` pointer) live in `ac/AGENTS.local.md`, which bootstrap never touches. The file carries no `<!-- agent:claude -->` / `<!-- agent:codex -->` blocks; the generator still supports them, and a new one needs a reason recorded in `docs/agents-md.md`. |
...
| `CLAUDE.md`, `agents/codex.md` | Yes, as derived artifacts | Generated by `scripts/generate_agent_configs.py` from `AGENTS.md`. After copying the shared `AGENTS.md` unchanged into `anywhere-agents`, run the generator locally to refresh these two files before commit. Bootstrap re-runs the generator on every consumer session. |
| `bootstrap/bootstrap.sh`, `bootstrap/bootstrap.ps1` | Yes, as-is | Byte-identical STRICT mirrors since the ac/bootstrap copy was re-synced to the canonical composer-aware version. Keep the generator invocation, `session_bootstrap.py` deploy block, and ac-to-aa migration block intact. |
...
| `scripts/render_banner.py`, `scripts/pack_identity.py` | Yes, as-is (STRICT) | Session banner renderer and its read-only pack-identity helper. Both entry points and the hook run them from the sparse clone, and the `aa` wheel vendors the pair beside the composer (aa-internal STRICT) so the `anywhere-agents` command renders the same calculation after its heal pass. |
...
| `skills/my-router/` | Yes, with content changes | The public version has concrete routing rules for the six shipped skills (`implement-review`, `ci-mockup-figure`, `readme-polish`, `prun`, `editable-figure`, plus `my-router` itself) with clear "extend this in your fork" guidance. Strip references to `nsf-*`, `usc-reimbursement`, `cs-paper-review`, etc. |
| `skills/editable-figure/` | Yes, as-is | Editable PowerPoint figure workflow for papers, proposals, and READMEs. STRICT byte-identical. It names the maintainer's own papers as the default palette source and their own awarded proposals as style exemplars, with locators into `NSF-Proposal-Template-Yue` and `internal-writing`. **Those locators are a deliberate maintainer decision, not a leak.** The preference is the content: the palette is the maintainer's, and a consumer without the referenced proposals simply has no locator to open, which the file already says. A sanitized public variant was built and then reverted, because it bought nothing and cost a permanent BY-DESIGN split plus a manual re-sanitization before every release. Leave the locators alone during the leak sweep; classify them as category (a), intentional. On 2026-09-25 the maintainer also authorized the bundled overview-figure gallery, including copied paper and awarded-proposal artwork, for this public mirror. Preserve each asset's recorded attribution and license. |
...
| `.claude/commands/` | Only for shipped skills | `implement-review.md`, `my-router.md`, `ci-mockup-figure.md`, `readme-polish.md`, `prun.md`, `editable-figure.md`. Since aa v0.4.0 these pointers are **pack-emitted** via `kind: skill` dispatch (see `bootstrap/packs.yaml` `aa-core-skills` entry; `prun.md` joined the pack in v0.7.7); they remain in both source trees for the PyYAML-missing fallback path but have **dropped from `scripts/check-parity.sh` STRICT**. Content still identical in practice; strict byte-identity is no longer enforced. |
...
| `tests/` | Yes, with edits | Remove tests for unshipped skills. Update `test_repo.py` to assert the six-skill shipped set and correct URLs. |
...
4. **Run the generator locally** after copying the shared `AGENTS.md` unchanged: `python scripts/generate_agent_configs.py --root .`. This refreshes `CLAUDE.md` and `agents/codex.md` so the committed artifacts match the committed `AGENTS.md`. Tests fail if they drift, and `tests/test_bootstrap_size.py` fails if any of the three files grows past its ceiling.
