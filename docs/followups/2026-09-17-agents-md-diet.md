...
- `scripts/remote-smoke.sh` against the published package (`npx --yes anywhere-agents`, 0.9.0) reports ALL CHECKS PASSED, twice, after four repairs to the box itself. `claude` was not logged in; the maintainer ran `/login` (the CLI then updated itself to 2.1.276). Codex's bubblewrap sandbox failed on every command (`bwrap: loopback: Failed RTM_NEWADDR`; `--unshare-user` alone gives `setting up uid map: Permission denied`) because Ubuntu 24.04 sets `kernel.apparmor_restrict_unprivileged_userns=1`; `/etc/apparmor.d/bwrap` now grants `userns` to `/usr/bin/bwrap` alone (`profile bwrap /usr/bin/bwrap flags=(unconfined) { userns, }`), loaded with `apparmor_parser -r`, and the global restriction is unchanged. A stale `@openai/codex` 0.133.0 under `~/.npm-global` shadowed the `/usr/local` copy whenever a script prepends `~/.npm-global/bin` to `PATH`, and the server answered its requests with `400 The '<model>' model requires a newer version of Codex`; both copies are 0.155.0 now. `~/.codex/config.toml` there gained `project_doc_max_bytes = 262144` (backup beside it); its `model` is `gpt-5.6-sol`, which the maintainer selected interactively that day.
...
- The ARM64 box's repairs are local state (a login, an AppArmor profile, two npm installs); nothing in either repo records them beyond this note.
