...
Reliability patch following v0.5.7's bundled-default flip. Single theme: any downstream user running plain `anywhere-agents` reaches a coherent end state with no internal-shaped errors and no stuck `CLAUDE.md`. Four concrete robustness fixes, no design changes (those land in v0.6.0).

- **Drift-gate skill-dir fix (concrete reproduction: usc-admin / usc-email, 2026-04-29).** When a pack's `output_paths` records a directory (e.g., `.claude/skills/implement-review/`), the composer's drift gate at `scripts/compose_packs.py:998-1162` builds `prior_pack_outputs` from `project_state.entries[*].output_path` (file-level keys) and walks the staging transaction at file-level. Files inside a recorded directory fall to `PRESTATE_UNMANAGED` and trigger `DriftAbort` whenever upstream has changed the bundled file content, because v0.5.3 adopt-on-match equates only the current pending sha256, not historical pack-output state. Reproduced on usc-admin / usc-email by running plain `anywhere-agents`: aa main 6d156fe → fc248ab landed a 33-line `skills/implement-review/SKILL.md` change, the gate aborted, `AGENTS.md` was not rewritten, and the post-compose generator never ran, so `CLAUDE.md` stayed at the prior compose's size (135390B vs the expected 66608B that matches `AGENTS.md` 70223B). Recovery message ("back up local edits, then rerun") is dev-speak; downstream users running `anywhere-agents` only see "pack composition failed" with no actionable next step. v0.5.8 fix: extend `prior_pack_outputs` to walk every file under recorded directory entries and seed it from on-disk sha256 at compose start, plus a per-pack `historical_input_sha256` ring so adopt-on-match accepts any prior known-good version.

- **Generator fallback on compose abort (defensive generated-file recovery).** Currently when the composer aborts for any reason (drift, network, lock, IO), the post-compose `generate_agent_configs.py` invocation may never run, especially on the default `anywhere-agents` path where `bootstrap/bootstrap.{sh,ps1}` exits before the generator step. Result: `CLAUDE.md` can stay decoupled from on-disk `AGENTS.md` indefinitely (usc-admin's typical end state: `CLAUDE.md` 135390B vs `AGENTS.md` 70223B). v0.5.8 runs the generator after every composer attempt in both bootstrap scripts and CLI composer callers. When composer rc=0, behavior is unchanged. When composer rc≠0, generated files refresh where possible, but the command still returns the composer rc and prints a recovery note saying pack composition did not complete.

- **`pack verify --fix --yes` materializes the current declared state automatically (v0.5.2 nice-to-have carried over from the maintainer's pack-deploy UX feedback).** Currently `--fix` writes yaml/lock rows but stops there; users must additionally run `bootstrap` or `anywhere-agents` to materialize content. The "register vs install" split violates pip / npm UX expectations. v0.5.8 makes `--fix` run the composer at the end to materialize the declared state. Scope is deliberately narrow: `--fix` does **not** apply prompt-policy update drift; that decision belongs to the v0.6.0 update-flow split (`pack verify --fix` vs `pack update`, Q1). Add `--no-deploy` opt-out for CI, offline, or inspect-only use (default: auto-deploy).

- **Git Bash on Windows fetches `.sh` (v0.5.2 nice-to-have item 3).** Currently `anywhere-agents` on Windows hard-fetches `bootstrap.ps1` regardless of which shell invoked it. Git Bash users see `No such file or directory` errors. v0.5.8 detects bash environment via `BASH_VERSION`, `MSYSTEM`, or shell process name and fetches `bootstrap.sh` accordingly. PowerShell detection (default) keeps current behavior. Fall back to `.ps1` when ambiguous.

**Consumer-facing change**: visible. `anywhere-agents` (no subcommand) on existing aa-bootstrapped projects no longer aborts when bundled skill content has changed since last compose. `pack verify --fix` now deploys end-to-end. Git Bash users on Windows can run the CLI without manual `.sh` fetching. CHANGELOG explicitly notes the four behavior changes.

**STRICT parity**: aa-internal mirror at `packages/pypi/anywhere_agents/composer/` updated for `compose_packs.py` changes (drift-gate fix). `cli.py` changes (items 2-4) are aa-only.
...
| aa v0.5.8 | **New entry required** — `bootstrap/bootstrap.{sh,ps1}` gain a finally-style generator pass that runs after every composer attempt | Document the four basic-command robustness fixes; extend post-code smoke contract with a drift-gate stale-skill-dir adoption test against a fixture project that reproduces the usc-admin failure mode | Update `docs/anywhere-agents-quickstart.md` § "Common gotchas" to remove the stale-CLAUDE.md workaround now that `anywhere-agents` recovers automatically; document the new `--no-deploy` flag on `pack verify --fix` |
...
| aa v0.5.8 | Basic command robustness | Drift-gate skill-dir fix (extends `prior_pack_outputs` + historical-sha adopt ring); generator fallback runs after every composer attempt in bootstrap scripts + CLI (preserves composer rc); `pack verify --fix --yes` materializes declared state automatically (with `--no-deploy` opt-out); Git Bash on Windows fetches `.sh` | Visible — `anywhere-agents` no longer leaves stale `CLAUDE.md` after a composer abort; `pack verify --fix` materializes end-to-end; Git Bash users on Windows can run the CLI |
