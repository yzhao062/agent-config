...
| [prun-executor-default](2026-09-08-prun-executor-default-measurement.md) | `prun` executor routing | Breadcrumb for the 2026-09-08 flip of prun's default from Codex to Sonnet, superseded 2026-09-10 when Codex was removed from `prun` entirely and Agy became the second pool. The operative rule lives in `skills/prun/SKILL.md`; this is the searchable pointer to it. The 09-08 measurement that motivated that flip was cut after four review rounds found it asserted more than it could support (cwd hash read as a run id, a switched token metric, repeated usage reports counted as turns, a mid-window snapshot read as a weekly result); a Sonnet-versus-Agy version is moot since 2026-09-13, when Sonnet was removed as well and `prun` became Agy-only because Sonnet workers billed the coordinator's own Claude account. | **Superseded 2026-09-10, then 2026-09-13**; kept as a pointer |
| [statusline-rollout-walk-junction-guard](2026-09-08-statusline-rollout-walk-junction-guard.md) | statusLine / `agent-quota` rollout discovery | The `os.scandir` walk skips symlinked directories but descends a Windows junction, which keeps the directory attribute `follow_symlinks=False` tests. Predates the walk (`glob` followed both). Closing it needs a `getattr` fallback for `DirEntry.is_junction()`, 3.12+, in two STRICT-shared copies. | Open; fold into the next change that touches the walk |
| [agents-md-diet](2026-09-17-agents-md-diet.md) | shared `AGENTS.md` rewrite (one baseline for ac and aa, 74.6 KB to about 23 KB) | Rule-preservation checklist: every line range of the old file mapped to KEPT, MOVED, SCRIPT, LOCAL, PACK, or DELETED with its destination; reviewers read it beside the diff. Phase 1 (ac rewrite, dispatcher `project_doc_max_bytes`, size gate, pointer descriptions, STRICT parity) in review 2026-09-17; the banner renderer, the aa byte-copy with its docs pages, and the agent-pack compact variants follow per `PLAN-agents-md-diet.md` |
